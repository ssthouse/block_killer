		...
		begin	
			if(!RSTn)
				begin
					...
					iDisplay_SCORE_X_Res <= 224;
					iDisplay_SCORE_Y_Res <= 20;
					
					iDisplay_S_T_X_Res <= 368;
					iDisplay_S_T_Y_Res <= 20;
				end
			else 
				begin
					x_pos <= clk_cnt - 10'd144;//a + b段的像素（640*480@60）
					y_pos <= line_cnt - 10'd33;//o + p段的像素（640*480@60）
					...
					else if(clk_cnt == 96)//a段有96个像素，过了之后hsync就拉高了
						begin
							hsync <= 1;
							clk_cnt <= clk_cnt + 10'd1;
						end
					else if(clk_cnt == 799)//整一行有799个像素，4个区域，满一行则移到下一行，行数目加1，列数目清零
						begin
							clk_cnt <= 0;
